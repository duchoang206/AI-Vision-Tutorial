...
5. [Vận hành & triển khai](#5-vận-hành--triển-khai)
6. [Kiến trúc hệ thống (UML)](#6-kiến-trúc-hệ-thống-uml)
7. [Ghi chú cho developer](#7-ghi-chú-cho-developer)
...
| ixwebsocket | Phát luồng WebSocket đa luồng, đang cải thiện độ trễ  |
...
Tại thư mục gốc `DetectRackProject/`, chạy lần lượt các lệnh sau để biên dịch mã nguồn thành file thực thi phục vụ chạy nền thuật toán:

```bash
# Tạo thư mục build riêng, tránh lẫn với mã nguồn
mkdir build && cd build

# Cấu hình project bằng CMake
cmake ..

# Biên dịch (dùng Release để tối ưu tốc độ suy diễn AI)
cmake --build . --config Release
```

Sau khi build xong, file thực thi `DetectRackProject` nằm trong `build/` (Linux) hoặc `build/Release/` (Windows). Đây là file dùng để chạy nền thuật toán detect ở bước tiếp theo.

> Nếu CMake báo không tìm thấy `libmodbus`, kiểm tra lại file `cmake/Findlibmodbus.cmake` và đảm bảo thư viện đã được cài đặt đúng đường dẫn hệ thống trước khi build lại.
...
### 5.2 Triển khai Web Dashboard (mạng LAN xưởng)
...
1. Lấy IP nội bộ của máy chạy Edge Server (ví dụ dùng `ipconfig` trên Windows hoặc `ip addr` trên Linux → `192.168.5.101`).
2. Tại thư mục chứa `index.html`, mở server tĩnh:
   ```bash
   python -m http.server 8000
   ```
3. Các máy tính/tablet khác trong cùng mạng LAN xưởng truy cập: `http://192.168.5.101:8000`

---

## 6. Kiến trúc hệ thống (UML)
...
### 6.3 Sơ đồ trình tự khởi động & đa luồng (Sequence Diagram)
...
### 6.4 Sơ đồ trạng thái vùng giám sát (State Diagram)
